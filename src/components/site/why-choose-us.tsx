...
export function WhyChooseUs() {
  return (
    <section className="relative py-20 lg:py-28 bg-[#0B1F3A] text-white overflow-hidden">
      <div className="absolute inset-0 bg-grid opacity-40" aria-hidden />
      <div className="absolute top-0 left-0 h-64 w-64 rounded-full bg-[#0056D2]/20 blur-3xl" aria-hidden />
      <div className="absolute bottom-0 right-0 h-80 w-80 rounded-full bg-[#4A90E2]/15 blur-3xl" aria-hidden />
          ...
          {whyChooseItems.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                className="group relative rounded-xl p-5 bg-white/5 border border-white/10 hover:bg-white/[0.08] hover:border-[#4A90E2]/40 transition-all backdrop-blur-sm"
              >
                <div className="absolute top-4 right-4 text-[10px] font-mono text-white/30">
                  {String(i + 1).padStart(2, "0")}
                </div>
                <div className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-[#0056D2] to-[#4A90E2] mb-3 shadow-md group-hover:scale-110 transition-transform">
        ...
        >
          {[
            ...
            <div key={s.label} className="text-center sm:text-left">
              <div className="font-[family-name:var(--font-poppins)] text-3xl lg:text-4xl font-extrabold bg-gradient-to-r from-[#4A90E2] to-white bg-clip-text text-transparent">
                {s.value}
              </div>
